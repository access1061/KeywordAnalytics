...
class KeywordCacheManager:
    ...
    def get_metrics(self, keyword, search_ttl_hours=72, doc_ttl_hours=24):
        ...
        return {
            ...
            "search_fresh": bool(search_updated_at and (now - search_updated_at < timedelta(hours=search_ttl_hours))),
            "doc_fresh": bool(doc_updated_at and (now - doc_updated_at < timedelta(hours=doc_ttl_hours))),
        }
...
class BackgroundWorker(QObject):
    ...
    DEMO_MAP = {
        "f_01": "0-12세 여자", "f_02": "13-18세 여자", "f_03": "19-24세 여자", "f_04": "25-29세 여자",
        "f_05": "30-34세 여자", "f_06": "35-39세 여자", "f_07": "40-44세 여자", "f_08": "45-49세 여자",
        "f_09": "50-54세 여자", "f_10": "55-59세 여자", "f_11": "60세- 여자", "m_01": "0-12세 남자",
        "m_02": "13-18세 남자", "m_03": "19-24세 남자", "m_04": "25-29세 남자", "m_05": "30-34세 남자",
        "m_06": "35-39세 남자", "m_07": "40-44세 남자", "m_08": "45-49세 남자", "m_09": "50-54세 남자",
    ...
    def _check_browser_ready(self, target_domain):
        ...
        if "nid.naver.com" in self.driver.current_url:
            raise ValueError("네이버 로그인 창에 머물러 있습니다. 로그인을 완료해주세요.")
                    ...
                    if d["data"] and isinstance(d["data"][0], dict) and "queryList" in d["data"][0]:
                        for r_idx, item in enumerate(d["data"][0]["queryList"], 1):
                            ...
                            all_trends_data.append({
                                "카테고리": cat, "순위": r_idx, "키워드": item.get("query", "N/A"), "순위변동": rc,
                            })
                except Exception as e:
                    ...
                    if d["data"] and isinstance(d["data"][0], dict) and "queryList" in d["data"][0]:
                        for r_idx, item in enumerate(d["data"][0]["queryList"], 1):
                            ...
                            all_age_trends.append({
                                "연령대": group_name, "순위": r_idx, "키워드": item.get("query", "N/A"), "순위변동": rc,
                            })
                except Exception as e:
                ...
                try:
                    ...
                    if "result" in r_json and (rows := r_json["result"].get("statDataList", [{}])[0].get("data", {}).get("rows")):
                        for dt, rank, cv, title, uri in zip(
                            rows.get("date", []), rows.get("rank", []), rows.get("cv", []),
                            rows.get("title", []), rows.get("uri", [])
                        ):
                            all_view.append({
    ...
    def _call_api_with_backoff(self, func, api_name, original_kw, max_retries=3):
        for attempt in range(max_retries):
            try:
                ...
                if status_code == 401: raise
                if status_code == 403: raise
                if status_code == 429:
                ...
                try:
                    ...
                    if cached["search_fresh"]:
                        ...
                        ad_data = self._call_api_with_backoff(
                            lambda: get_naver_ad_keywords(kw_api, api_keys["ads_key"], api_keys["ads_secret"], api_keys["customer_id"], session),
                            "광고 API", original_kw,
                        )
                    ...
                    if cached["doc_fresh"]:
                        ...
                        post_count = self._call_api_with_backoff(
                            lambda: get_blog_post_count(kw_api, api_keys["client_id"], api_keys["client_secret"], session),
                            "블로그 검색 API", original_kw,
                        )
                    ...
                    analysis_results.append({
                        "분류": cat, "키워드": original_kw, "총검색량": tot_search,
                        "총문서수": post_count, "기회지수": round(opp_idx, 2),
                    })
                except ApiCallError as e:
                    if e.status_code == 401:
                        self.log.emit("ERROR", f"'{original_kw}' 오류: API 키/시크릿이 잘못되었습니다 (401).")
                    elif e.status_code == 403:
                        self.log.emit("ERROR", f"'{original_kw}' 오류: API 권한이 없습니다 (403).")
                    elif e.status_code == 429:
                        self.log.emit("WARNING", f"'{original_kw}' 오류: 호출 제한이 계속 발생하여 이번 키워드는 건너뜁니다.")
                    else:
...
class KeywordApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.cached_data = {
            "trends": None, "age_trends": None, "main_inflow": None,
            "blog_views": {}, "analysis": {}, "auto": {},
        }
        ...
        if missing_keys:
            ...
            self.log_message("WARNING", "해당 경로에 api.env 파일이 존재하는지, 내용이 정확한지 확인해주세요.")
        else:
    ...
    def check_views_cache_state(self):
        ...
        self.update_button_style(self.fetch_blog_views_button, is_cached, "켜진 창에서 조회수 순위 가져오기")
    ...
    def create_settings_bar(self, parent_layout):
        ...
        self.auth_button.setStyleSheet("background-color: #007BFF; font-weight: bold; padding: 8px;")
    ...
    def create_trend_fetch_tab(self):
        ...
        self.fetch_trends_button.setStyleSheet("background-color: #28A745; font-weight: bold;")
        self.fetch_age_trends_button.setStyleSheet("background-color: #28A745; font-weight: bold;")
    ...
    def create_analysis_tab(self):
        ...
        self.result_table.setHorizontalHeaderLabels(["분류", "키워드", "총검색량", "총문서수", "기회지수"])
    ...
    def create_naver_main_tab(self):
        ...
        layout = QVBoxLayout(tab)
        control_layout = QHBoxLayout()
        self.fetch_main_content_button = QPushButton("켜진 창에서 메인 유입콘텐츠 가져오기")
        self.fetch_main_content_button.setStyleSheet("background-color: #28A745; font-weight: bold;")
    ...
    def create_blog_views_tab(self):
        ...
        self.update_button_style(self.fetch_blog_views_button, False, "켜진 창에서 조회수 순위 가져오기", "background-color: #28A745; font-weight: bold;")
    ...
    def reset_ui(self):
        ...
        self.update_button_style(self.fetch_trends_button, False, "2-1. 켜진 창에서 주제별 수집", "background-color: #28A745; font-weight: bold;")
        self.update_button_style(self.fetch_age_trends_button, False, "2-2. 켜진 창에서 연령별 수집", "background-color: #28A745; font-weight: bold;")
        self.update_button_style(self.fetch_main_content_button, False, "켜진 창에서 메인 유입콘텐츠 가져오기", "background-color: #28A745; font-weight: bold;")
        self.check_views_cache_state()
    ...
    def on_trend_fetching_finished(self, data):
        ...
        if data:
            ...
            self.update_button_style(self.fetch_trends_button, True, "2-1. 켜진 창에서 주제별 수집", "background-color: #28A745; font-weight: bold;")
    ...
    def on_age_trend_fetching_finished(self, data):
        ...
        if data:
            ...
            self.update_button_style(self.fetch_age_trends_button, True, "2-2. 켜진 창에서 연령별 수집", "background-color: #28A745; font-weight: bold;")
    ...
    def on_naver_main_finished(self, res):
        ...
        if res:
            ...
            self.update_button_style(self.fetch_main_content_button, True, "켜진 창에서 메인 유입콘텐츠 가져오기", "background-color: #28A745; font-weight: bold;")
